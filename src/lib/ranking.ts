...
function calculateFreshnessScore(createdAt: Date | null, maxAgeDays: number = 30): number {
...
function calculateScore(offer: OfferWithAdvertiser): number {
  ...
  const score = (commission * 0.3) + (clicksScore * 0.25) + (verifiedScore * 0.2) + (expiryScore * 0.15) + (freshnessScore * 0.1);
  
  return Math.round(score * 100) / 100;
}

export async function getRankedOffers(limit = 50, category?: string): Promise<OfferWithAdvertiser[]> {
  ...
  const offers = await query<OfferWithAdvertiser>(
    ...
    WHERE o.is_expired = 0 AND a.status = 'active' ${categoryFilter}
    GROUP BY o.id, a.id, c.clicks
    ORDER BY 
      CASE WHEN o.is_verified = 1 THEN 0 ELSE 1 END,
